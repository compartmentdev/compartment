...
description: Implement repo changes by starting with preflight feature review, then editing the smallest owning surface on one canonical path and running only diff-triggered checks.
---
...
2. Run `$preflight-feature-review` with the request, intended outcome, known scope, and visible constraints. Let that skill own its review agents. Do not edit before its brief returns.
...
8. After implementation, run owning-package `pnpm lint`, `pnpm typecheck`, and `pnpm test` when applicable, plus only the diff-triggered special checks. Do not manually run repo-wide `pnpm check`, `pnpm check:unused`, `pnpm check:unused:runtime`, or `pnpm check:ci` unless the task explicitly requires them.
9. Close out with the entry file to read first, checks run, remaining unrun validation, and the user-visible verification flow.
