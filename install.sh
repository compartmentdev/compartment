...
if [ "$release_repository" = "compartmentdev/compartment" ]; then
  installer_url="https://compartment.dev/install.sh"
else
  installer_url="https://raw.githubusercontent.com/${release_repository}/kubernetes/install.sh"
fi
    ...
    case "$oras_resolve_error" in
      *"not found"* | *"manifest unknown"*)
        if [ "$channel" = "kubernetes" ] && [ "$version_argument" = "1" ]; then
          ...
          printf 'curl -fsSL %s | sh -s -- --channel kubernetes\n' "$installer_url" >&2
          ...
          printf 'Images for %s are still publishing. Install the latest fully published kubernetes build with:\n' \
            "$resolved_release_tag" >&2
          printf 'curl -fsSL %s | sh -s -- --channel kubernetes --version %s\n' \
            "$installer_url" "$last_published_release_tag" >&2
        else
          ...
          printf 'curl -fsSL %s | sh -s -- --channel kubernetes --version sha-COMMIT_SHA\n' "$installer_url" >&2
        fi
        ;;
      *)
