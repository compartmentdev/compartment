...
oras_version="1.3.3"
main_cli_certificate_identity="https://github.com/compartmentdev/compartment/.github/workflows/publish-self-hosted-main.yml@refs/heads/main"
main_cli_certificate_oidc_issuer="https://token.actions.githubusercontent.com"
channel="latest"
version=""
version_argument="0"
channel_argument="0"
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    --channel)
      if [ "$#" -lt 2 ]; then
        printf 'Expected --channel <latest|main>.\n' >&2
        exit 1
      fi
      channel="$2"
      channel_argument="1"
  ...
  case "$channel" in
    main)
      ...
      if [ "$validated_version" != "$version" ]; then
        printf 'Invalid version for the main channel: %s. Expected sha- followed by 40 lowercase hexadecimal characters.\n' \
          "$version" >&2
        exit 1
      fi
      ;;
...
resolve_last_published_main_release_tag() {
  successful_runs_url="https://api.github.com/repos/${release_repository}/actions/workflows/publish-self-hosted-main.yml/runs?branch=main&status=success&per_page=1"
  ...
  if ! "$cosign_command" verify \
    --new-bundle-format \
    --certificate-identity "$main_cli_certificate_identity" \
    --certificate-oidc-issuer "$main_cli_certificate_oidc_issuer" \
...
prepare_main_cli_tools() {
  ...
  case "$tools_target_arch" in
    ...
    arm64)
      ...
      ;;
    *)
      printf 'Unsupported main CLI tool architecture: %s\n' "$tools_target_arch" >&2
      exit 1
      ;;
  esac
  ...
  if ! cli_manifest_digest="$("$oras_command" resolve "$cli_tag_ref" 2>"$oras_resolve_error_path")"; then
    ...
    case "$oras_resolve_error" in
      *"not found"* | *"manifest unknown"*)
        if [ "$version_argument" = "1" ]; then
          printf 'Main CLI image tag %s was not found in the registry. Check that the build was published and that the version is not mistyped.\n' \
            "$resolved_release_tag" >&2
          printf 'To install the current main channel tip instead, omit --version:\n' >&2
          printf '%s --channel main\n' "$installer_command" >&2
        elif last_published_release_tag="$(resolve_last_published_main_release_tag)"; then
          printf 'Images for %s are still publishing. Install the latest fully published main build with:\n' \
            "$resolved_release_tag" >&2
          printf '%s --channel main --version %s\n' \
          ...
          printf 'Images for %s are not published, and the installer could not verify a fallback automatically.\n' \
            "$resolved_release_tag" >&2
          printf 'Open https://github.com/%s/actions/workflows/publish-self-hosted-main.yml, select the latest successful run, and copy its full commit SHA into:\n' \
            "$release_repository" >&2
          printf '%s --channel main --version sha-COMMIT_SHA\n' "$installer_command" >&2
        fi
        ;;
      *)
        printf 'Failed to resolve main CLI image %s. Check registry access and retry.\n' "$cli_tag_ref" >&2
        ;;
    esac
    return 1
  fi
...
verify_main_cli_artifact() {
  ...
  if ! "$cosign_command" verify \
    ...
    printf 'Failed to verify main CLI artifact %s\n' "$cli_digest_ref" >&2
    exit 1
  fi
  if [ "$verbose" = "1" ]; then
    cat "$verification_output_path" >&2
...
if [ "$channel" = "main" ]; then
  ...
  resolved_main_commit_sha="${resolved_release_tag#sha-}"
  prepare_main_cli_tools "${temp_directory}/tools" "$target_os" "$target_arch"
  cli_tag_ref="${cli_oci_repository}:${resolved_release_tag}"
  cli_digest_ref="$(resolve_main_cli_digest_ref "$cli_tag_ref")"
  verify_main_cli_artifact "$cli_digest_ref" "$resolved_main_commit_sha"
