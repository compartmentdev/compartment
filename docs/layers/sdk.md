...
SDK owns the typed client surface over shared public and internal contracts.

- Owns request building, response parsing, and transport defaults.
- Transport defaults may include API URL, session token, current organization slug, and internal token.
...
- Must not depend on `api`, `cli`, or `test-support`.
