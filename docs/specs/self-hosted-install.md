...
- Self-hosted source builds use the published Compartment builder image, which runs rootful BuildKit listening only on `unix:///run/buildkit/buildkitd.sock`; the worker reaches BuildKit through a named Docker volume, not TCP. The BuildKit socket group is the fixed runtime GID `10001`.
