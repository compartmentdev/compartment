...
describe.sequential('test database run lifecycle', (): void => {
  it('preserves an active run while removing an unlocked orphan', async (): Promise<void> => {
  ...
  it('serializes concurrent cleaners for the same orphaned run', async (): Promise<void> => {
    ...
    try {
      ...
      await Promise.all([cleanupStaleTestDatabaseRuns(testDatabaseUrl), cleanupStaleTestDatabaseRuns(testDatabaseUrl)]);
  ...
  it('drops every database owned by the exact run during teardown', async (): Promise<void> => {
