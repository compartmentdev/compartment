...
function immutableKubeName(prefix: string, immutableId: string): string {
  ...
  if (trimmedId.length === 0) throw new Error('Kubernetes names require a non-empty immutable ID.');
  ...
  if (!/^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/.test(name)) throw new Error(`Invalid immutable Kubernetes name: ${name}`);
  return name;
}
