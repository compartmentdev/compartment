...
import { runCommand, runCommandWithInputAndTimeout, runCommandWithTimeout } from '../src/command-runner';
...
describe('command runner timeout', (): void => {
  ...
  it('terminates an input command at its hard deadline', async (): Promise<void> => {
    const result: CommandResult = await runCommandWithInputAndTimeout(
      [process.execPath, '-e', 'process.stdin.resume(); setInterval(() => {}, 1_000)'],
      'probe',
      100,
    );
