...
describe('production install option validation', (): void => {
  it('defaults to a managed domain and the production broker', (): void => {
    expect(resolveKubernetesInstallCommandOptions({ output: 'json', values: 'compartment-values.yaml' })).toMatchObject(
      {
  ...
  it('requires the Console host to match an explicit base domain', (): void => {
    expect((): void => {
      resolveKubernetesInstallCommandOptions(createOptions('apps.example.com', 'https://console.other.example.com'));
