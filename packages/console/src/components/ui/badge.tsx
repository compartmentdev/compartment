...
function readBaseBadgeClassName(): string {
  return 'inline-flex items-center gap-1.5 rounded-control px-2 py-1 text-[11px] font-medium transition-colors';
}
...
function readBadgeVariantClassName(variant: BadgeVariant): string {
  switch (variant) {
    ...
    case 'info':
      return "h-6 shrink-0 justify-center rounded-control border border-[var(--opacity-info-lighter)] bg-[var(--opacity-info-lighter)] px-2 py-1 text-[13px] font-semibold leading-5 text-info [font-variation-settings:'opsz'_14]";
    case 'outline':
      return 'border border-border/80 bg-background/70 text-foreground';
    case 'soft':
      return "h-6 shrink-0 justify-center rounded-control border border-[var(--tag-bg-default)] bg-[var(--tag-bg-default)] px-2 py-1 text-[13px] font-semibold leading-5 text-[var(--tag-text-default)] [font-variation-settings:'opsz'_14]";
    case 'success':
