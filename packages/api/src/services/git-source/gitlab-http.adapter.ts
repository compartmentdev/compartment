...
export class GitLabPaginationLimitError extends Error {
  public constructor(
    public readonly path: string,
    public readonly pageCap: number,
  ) {
    super(
      `GitLab pagination exceeded the ${String(pageCap)} page safety cap for ${path}. Narrow the repository scope and retry.`,
    );
    this.name = 'GitLabPaginationLimitError';
  }
}
...
export class GitLabHttpClient {
  ...
  public async requestPages<T>(input: GitLabRequestInput, pageCap: number): Promise<T[]> {
    ...
    throw new GitLabPaginationLimitError(input.path, pageCap);
  }
...
export function readGitLabHttpStatus(error: Error | undefined): number | null {
  return error instanceof GitLabHttpError ? error.status : null;
}
