...
describe('GitLab webhook adapter', (): void => {
  ...
  it('marks pushes incomplete when the total or any commit file arrays are missing', (): void => {
    expect(parseGitLabPushPayload(buildPush({}))?.changedFilesComplete).toBe(false);
    expect(
      parseGitLabPushPayload(
        buildPush({ commits: [{ added: ['added.txt'], removed: ['removed.txt'] }], total_commits_count: 1 }),
