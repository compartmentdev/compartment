...
describe('Phase 0 API integration organization user concurrency', (): void => {
  ...
  it('consumes each invitation token at most once under concurrent activation', async (): Promise<void> => {
    ...
    try {
      ...
      const firstActivationPromise: Promise<LightMyRequestResponse> = activateUser(activationPayload);
      const secondActivationPromise: Promise<LightMyRequestResponse> = activateUser(activationPayload);
...
async function activateUser(payload: ActivationPayload): Promise<LightMyRequestResponse> {
