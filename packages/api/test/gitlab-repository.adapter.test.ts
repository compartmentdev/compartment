...
import { GitLabHttpClient, GitLabPaginationLimitError } from '../src/services/git-source/gitlab-http.adapter';
import { listGitLabProjects, readGitLabTree } from '../src/services/git-source/gitlab-repository.adapter';
...
describe('GitLab repository adapter', (): void => {
  ...
  it('preserves actionable pagination failures for project listing', async (): Promise<void> => {
    const client: GitLabHttpClient = Object.create(GitLabHttpClient.prototype) as GitLabHttpClient;
    vi.spyOn(client, 'requestPages').mockRejectedValue(new GitLabPaginationLimitError('/projects', 50));

    await expect(listGitLabProjects(client)).rejects.toThrow('Narrow the repository scope and retry');
  });

  it('preserves actionable pagination failures for repository trees', async (): Promise<void> => {
    const client: GitLabHttpClient = Object.create(GitLabHttpClient.prototype) as GitLabHttpClient;
    vi.spyOn(client, 'requestPages').mockRejectedValue(
      new GitLabPaginationLimitError('/projects/42/repository/tree', 50),
    );

    await expect(readGitLabTree(client, '42', 'main')).rejects.toThrow('Narrow the repository scope and retry');
  });
});
