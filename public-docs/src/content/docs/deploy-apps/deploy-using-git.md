...
- a provider registration matching the remote host;
- `github.com` or `gitlab.com`;
- otherwise an actionable error.

The token is read only after the CLI resolves GitLab. For full token, expiry, self-hosted, subgroup, limit, and security guidance, read [Connect GitLab repositories](/deploy-apps/gitlab-git-sources/).
...
compartment source connect git --provider gitlab
