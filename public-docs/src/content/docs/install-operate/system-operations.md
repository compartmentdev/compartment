...
When you use `system update`, Compartment reads the stored install state and keeps the selected image source and image registry unless you override them again. New registry installs use GitHub Container Registry by default. Older registry install states without a stored image registry use GitHub Container Registry on the next registry update unless you run `sudo compartment system update --image-registry docker-hub`; older local-image states keep Docker Hub-style local image names unless you choose another registry explicitly.

`system update` expects the canonical self-hosted env file generated by current `compartment install`: `.env.self-hosted`
with the self-hosted install secrets, registry credentials, required canonical variables, and canonical host socket paths.
