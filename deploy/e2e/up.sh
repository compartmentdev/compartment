...
for _ in $(seq 1 60); do
  console_status="$(curl --silent --output /dev/null --write-out '%{http_code}' "http://console.localhost:${HOST_HTTP_PORT}/" || true)"
...
if [[ "${console_status}" != 302 ]]; then
  echo "Console readiness failed: expected HTTP 302, got ${console_status:-no response}." >&2
  exit 1
fi
