...
function renderDescriptorSchemaPage(response) {
  ...
  return renderMarkdown([
    ...
    `- Restart policies: ${response.rules.restartPolicies.join(', ')}.`,
    `- Restart max-retry policies: ${response.rules.restartMaxRetriesPolicies.join(', ')}.`,
    ...
    `- Resource restart fields: ${response.rules.resourceRestartFields.join(', ')}.`,
    `- Resource restart policies: ${response.rules.resourceRestartPolicies.join(', ')}.`,
    ...
    `- Run restart policy defaults to \`${response.defaults.serviceRun.restart.policy}\`.`,
    `- Run restart maxRetries defaults to ${response.defaults.serviceRun.restart.maxRetries === undefined ? 'unset' : `\`${response.defaults.serviceRun.restart.maxRetries.toString()}\``}.`,
    ...
    `- Resource restart policy defaults to \`${response.defaults.resourceRestart.policy}\`.`,
